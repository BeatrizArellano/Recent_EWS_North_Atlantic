"""
Assessing the significance of the measured trend for different combinations of bandwidth and window length
"""
import numpy as np
import pandas as pd
import sys
...
for rec in sel_rec.columns:
    ...
    np.save(f'output/significant_combinations/bW_vs_{rec}_recent.npy', bW_vs)
    np.save(f'output/significant_combinations/wL_vs_{rec}_recent.npy', wL_vs)
    np.save(f'output/significant_combinations/par1_{rec}_recent.npy', par1)
